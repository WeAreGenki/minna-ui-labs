...
import minnaUi, { aliasResolve } from '@minna-ui/postcss-plugin';
...
/** @typedef {import('@minna-ui/postcss-plugin').ImportAlias} ImportAlias */

/**
 * @typedef {object} PluginOptions
 * @property {boolean} [debug] - Show useful debugging feedback (e.g.
 * unresolved variables).
 * @property {ImportAlias} [importAlias] - A map of import aliases. Given a
 * matching regex key, will replace the import path with the value. Example:
 * `{ '^##\\/(.*)$': 'src/$1' }`.
 * @property {string[]} [importPaths] - A list of extra paths to search when
 * resolving `@import` rules in CSS. First, imports will try to resolve
 * according to the [CSS Import Resolve spec](https://jonathantneal.github.io/css-import-resolve/)
 * and then try again with each of the `importPaths`.
 * @property {boolean} [optimize] - Perform optimisations to reduce output file
 * size and minimise runtime style computation.
 * @property {boolean} [unsafe] - Apply potentially unsafe transformations
 * (e.g. combining same `@media`).
 * @property {any[]} [options] - Any other options will be passed to all
 * PostCSS plugins and to the `nanocss` preset options.</br></br>This can be
 * particularly powerful if you need to pass options when using the `@use`
 * rule — use the plugin name as a key, as shown in the [postcss-use docs](https://github.com/postcss/postcss-use#options-1).
 */
...
export default postcss.plugin(
   ...
   * @returns {import('postcss').PluginInitializer}
   */
  ({
    debug = true,
    importAlias = { '^##\\/(.*)$': 'src/$1' },
